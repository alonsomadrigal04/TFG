...
public partial class GameManager : Node
{
    ...
    public override void _Process(double delta)
    {
        if (DialogManager.IsSpeaking && Input.IsActionJustPressed("ui_accept") && !ActionBus.IsBusy)
        {
            ...
            DialogManager.textTyper.isAcceptPressed = true;
        }
        if (Input.IsActionJustReleased("ui_accept"))
        {
            DialogManager.textTyper.isAcceptPressed = false;
        }
        if(DialogManager.IsSpeaking && Input.IsActionPressed("fasterDialog"))
        {
            DialogManager.textTyper.isFasterDialog = true;
        }
        if(DialogManager.textTyper.isSentenceComplete && Input.IsActionPressed("fasterDialog"))
        {
            DialogManager.OnNextRequested();
        }
        if (DialogManager.IsSpeaking && Input.IsActionJustReleased("fasterDialog"))
        {
            DialogManager.textTyper.isFasterDialog = false;
        }
    }
